    ...
    for(let key = 0; key < cart.length; key++){
        ...
        // if(key == i){
            // for(let key1 = 0; key1 < JSON.parse(cart[key][1]).length; key1++){
                ...
                document.getElementById("korz__order"+key).innerHTML += "<div id='korz__order-clear"+key+"' class='korz__order-clear'><button type='submit'><img src='images/korz-clear.png' alt='Удалить'></button></div><div id='korz__container"+key+"' class='korz__container' data-price='"+JSON.parse(cart[key][1])[3][1]+"' data-id='"+JSON.parse(cart[key][1])[0][1]+"'></div>";
